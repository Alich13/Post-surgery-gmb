...
```{r}
dataset.gbm
table(dataset.gbm@meta.data$cond.day)
...
## Regroup Ribo S and L .

Ribosomal genes seem to share high information levels with hypoxia metadata. for readability purposes we chose to collapse these genes . See figure below .![Ribo genes](images/Tumor_MI_heatmap_filtered.withRibo.png){width="434"}
...
dataset.gbm <- AddMetaData(
  object = dataset.gbm,
  metadata = mean_rl_expression, # Mean expression for each cell
...
# debug 
# dataset.gbm
# ribosomal_s_matrix[1:3,1:3]
# mean_rs_expression[1:3]
# ribosomal_l_matrix[1:3,1:3]
# mean_rl_expression[1:3]
# colMeans(ribosomal_l_matrix)[1:3]
# rowMeans(ribosomal_l_matrix)[1:3]
# dataset.gbm@meta.data
...
The list of ligands-receptor was generated with Liana+ - `/Users/alichemkhi/Desktop/data/LianaCCC.csv`\` .
...
if (ENABLE_MI_SELECTION) {
  ...
  MI_senders_genes <- causalCCC.MIselection(data_input = dataset.gbm,
                                 ...
                                 plot = T)
  save(MI_senders_genes, file = "../output/MI_tables_noRibo/MI_senders_genes.RData") 
}else {
  MI_senders_genes=load("../output/MI_tables_noRibo/MI_senders_genes.RData")
}
...
MI_receivers_genes <- causalCCC.MIselection(data_input = dataset.gbm,
                                 ...
                                 plot = T)
save(MI_receivers_genes, file = "../output/MI_tables_noRibo/MI_receivers_genes.RData") 
}else {
  MI_receivers_genes=load("../output/MI_tables_noRibo/MI_receivers_genes.RData")
}
